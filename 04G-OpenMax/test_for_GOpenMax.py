...
def test(model, device, test_loader, weibull_model, threshold, **options):
    ...
    for i in range(0, len(labels)):
        if labels[i] > len(options['train_typelist'])-1:
            labels[i] = len(options['train_typelist'])-1
            labels_open.append(0)
        elif labels[i] == len(options['train_typelist'])-1:
            labels_open.append(0)
        else:
    ...
    predict = np.argmax(score_softmax, axis=1)
    predict[predict==len(options['train_typelist'])] = len(options['train_typelist'])-1
    ...
    for i in range(len(unknown_index)):
        labels_open_pred[unknown_index[i]] = len(options['train_typelist'])-1
    ...
    print(f"G-OpenMax AUROC is %.3f" % (100*AUROC_bin))
    print(f"G-OpenMax accuracy is %.3f" % (100*accuracy))
    print(f"G-OpenMax Weighted F1 is %.3f" % (100*f1_weighted))
    print(f"G-OpenMax Macro F1 is %.3f" % (100*f1_macro))
...
def main(options):
    ...
    if options['dataset'] == 'radar':
        model = RFFE(in_channels=2, num_class=len(options['train_typelist']))
    elif options['dataset'] == 'ADSB':
        model = RFFE(in_channels=3, num_class=len(options['train_typelist']))
    options.update(
        {
            'use_gpu': True,
        }
...
if __name__ == '__main__':
    ...
    from split2 import train_splits as train_splits
    from split2 import test_splits as test_splits
    ...
    for j in range(len(test_list)):
        ...
        result_model_path = './log/NK+1/' + options['model_name'] + '/' + options['dataset']
        ...
        for i in range(len(train_splits[options['dataset']])):
            ...
            weight_path = './log/NK+1/' + options['model_name'] + '/' + options['dataset'] + '/' + files[i] + '/result_model/'+options['model_name']+'.pth'
            options.update(
                {
