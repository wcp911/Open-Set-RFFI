...
def train(model, device, criterion, optimizer, train_loader, epoch,  **options):
    model.train()
    running_loss = 0.0
    loss_list = []
    iteration = 0
...
def val(model, device, val_loader, epoch, **options):
    ...
    #signal_list = np.unique(np.int64(label_real.cpu().numpy()))
...
def main(options):
    ...
    for epoch in range(maxepoch):
        ...
        loss_train.extend(train(model, device, criterion, optimizer, trainloader, epoch, **options))
        ...
        if is_best:
            ...
            pthFileFullName = save_networks(model, options['result_model_path'], options['model_name'])
...
if __name__ == '__main__':
    ...
    # from split2 import train_splits as splits
    ...
    for j in range(len(train_list)):
        ...
        for i in range(len(splits[options['dataset']])):
            ...
            result_model_path = './log/NK/' + options['model_name'] + '/' + options['dataset'] + '/' + str(current_time) + '/result_model'
            confusion_matrix_path = './log/NK/'+ options['model_name'] + '/'+ options['dataset'] + '/' + str(current_time) + '/confusion_matrix'
            log_path = './log/NK/'+ options['model_name'] + '/'+ options['dataset'] + '/' + str(current_time)
            options.update(
                {
