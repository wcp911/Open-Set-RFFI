...
def train(model, device, criterion, optimizer, train_loader, epoch, center_loss,  **options):
    lr_cent = 0.001
    alpha = 0.1
    ...
    for batch_idx, (inputs, target_name, target) in enumerate(train_loader, 0):
        ...
        loss = center_loss(outputs, target) * alpha + criterion(outputs, target)

        loss.backward()
        for param in center_loss.parameters():
            param.grad.data *= (lr_cent / (alpha * options['lr']))  # options['lr']
...
def save_networks(networks, resultpath, netname):
...
def main(options):
    ...
    Center_Loss = CenterLoss(num_classes=len(options['train_typelist']), feat_dim=len(options['train_typelist']))
    ...
    for epoch in range(maxepoch):
        ...
        loss_train.extend(train(model, device, criterion, optimizer, trainloader, epoch, Center_Loss, **options))
        ...
        if is_best:
            ...
            pthFileFullName = save_networks(model, options['result_model_path'], netname=options['model_name'])
...
if __name__ == '__main__':
    ...
    for j in range(len(train_list)):
        ...
        for i in range(len(splits[options['dataset']])):
            ...
            result_model_path = './log/OpenMax+CL/' + options['model_name'] + '/' + options['dataset'] + '/' + str(current_time) + '/result_model'
            confusion_matrix_path = './log/OpenMax+CL/' + options['model_name'] + '/' + options['dataset'] + '/' + str(current_time) + '/confusion_matrix'
            log_path = './log/OpenMax+CL/' + options['model_name'] + '/' + options['dataset'] + '/' + str(current_time)
            options.update(
                {
