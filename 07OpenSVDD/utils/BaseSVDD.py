...
class BaseSVDD(BaseEstimator, OutlierMixin):
    ...
    def fit(self, X, y=None, weight=None):
        ...
        self.display = 'off'
        self.predicted_y_, self.distance = self.predict(self.X, self.y)
        self.accuracy = accuracy_score(self.y, self.predicted_y_)
        # self.positive_accuracy = accuracy_score(self.y[:int(len(self.y)/2), :], self.predicted_y_[:int(len(self.y)/2), :])
        # if self.positive_accuracy < 0.5:
        #     self.distance = np.array(self.distance[:int(len(self.y)/2), :]).squeeze()
        #     max_value = self.distance.max()
        #     min_value = self.distance.min()  # 求每一行最小值的列索引
        #     self.radius = (max_value - min_value)*0.5 + min_value
        #     # self.distance.sort()
        #     # self.radius = float(self.distance[int(len(self.distance)*0.99)])
    ...
    def predict(self, X, y=None):
        ...
        return results['predicted_y'], results['distance']
