...
parser.add_argument('--best_acc_val', type=float, default=0)
...
def train(model, device, criterion, criterion2, optimizer, train_loader, epoch, center_loss,  **options):
    lr_cent = 0.001   # lr_cent is learning rate for center loss
    alpha = 0.1
    model.train()
    running_loss = 0.0
    loss_list = []
    ...
    for batch_idx, (inputs, target_name, target) in enumerate(train_loader, 0):
        B, C, L = inputs.shape
        noise = torch.normal(mean, std, size=(B, C, L))
        inputs = inputs + 0.1*noise
        inputs, target = inputs.to(device), target.to(device)
        optimizer.zero_grad()
        encoder_out, classifier_out, decoder_out = model(inputs)
        loss = alpha*center_loss(encoder_out, target) + criterion(classifier_out, target) + 0.5*criterion2(inputs, decoder_out)
        loss.backward()
        for param in center_loss.parameters():
            param.grad.data *= (lr_cent / (alpha * options['lr']))
        optimizer.step()
...
def val(model, device, val_loader, epoch, **options):
    ...
    with torch.no_grad():
        for data in val_loader:
            ...
            inputs, target = inputs.to(device), target.to(device)
            encoder_out, classifier_out, decoder_out = model(inputs)
            _, predicted = torch.max(classifier_out.data, dim=1)
...
def main(options):
    ...
    if options['dataset'] == 'radar':
        model = AutoEncoder(in_channels=2, num_class=len(options['train_typelist']))
    else:
        model = AutoEncoder(in_channels=3, num_class=len(options['train_typelist']))
    print(model)
    ...
    Center_Loss = CenterLoss(num_classes=len(options['train_typelist']), feat_dim=1024)
    ...
    for epoch in range(maxepoch):
        ...
        loss_train.extend(train(model, device, criterion, criterion2, optimizer, trainloader, epoch, Center_Loss, **options))
...
if __name__ == '__main__':
    ...
    for j in range(len(train_list)):
        ...
        for i in range(len(splits[options['dataset']])):
            ...
            result_model_path = './log/MeDAE/' + options['dataset'] + '/' + str(current_time) + '/result_model'
            confusion_matrix_path = './log/MeDAE/'+ options['dataset'] + '/' + str(current_time) + '/confusion_matrix'
            log_path = './log/MeDAE/'+ options['dataset'] + '/' + str(current_time)
            options.update(
                {
