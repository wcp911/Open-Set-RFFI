...
from train_for_MeDAE import seed_everything
...
parser.add_argument('--snr_range', default=[20, 30])
...
def test(model, device, test_loader, center, threshold, **options):
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            predict.append(classifier_out.cpu().numpy())
            scores.append(encoder_out.cpu().numpy())
            labels.extend(target)
    scores = np.concatenate(scores, 0)
    predict = np.concatenate(predict, 0)
    labels = np.array(labels)
    # np.savez('./log/DNAE_feature.npz', outputs_list=predict, label_list=labels)
    ...
    print(f"MeDAE AUROC is %.3f" % (100*AUROC_bin))
    print(f"MeDAE accuracy is %.3f" % (100*accuracy))
    print(f"MeDAE Weighted F1 is %.3f" % (100*f1_weighted))
    print(f"MeDAE Macro F1 is %.3f" % (100*f1_macro))
...
def get_center(model, device, data_loader, **options):
    ...
    threshold = sorted_values[int(0.97 * len(scores))]
...
if __name__ == '__main__':
    ...
    for j in range(len(test_list)):
        ...
        result_model_path = './log/MeDAE/' + options['dataset']
        ...
        for i in range(len(train_splits[options['dataset']])):
            ...
            weight_path = './log/MeDAE/' + options['dataset'] + '/' + files[i] + '/result_model/'+options['model_name']+'.pth'
            options.update(
                {
                    ...
                    'data_root': 'D:/wu/IEEEWCL实验/00data/',
                    'weibull_tail': 20,
